...
export default function FavoritesPage() {
    ...
    const favoritesMapRef = useRef(new Map<string, Property>());
    ...
    const handleFavoriteChange = useCallback((id: string, nextState: boolean) => {
        const property = favoritesMapRef.current.get(id);
        if (!property && nextState) return;
        ...
        setFavorites((current) => {
            ...
            // We only reach here if nextState is true and property exists
            return [property as Property, ...current];
        });
    }, []);
